...
export default async function SendContactValidationCode({...props}:sendContactValidationCode) {
 const cookieInfos = cookies().get("userProfile")

 return (
    <SendContactValidationCodeForm welcome={props.searchParams.welcome == "true" ? true : false} cookieName={cookieInfos?.name} cookieVal={cookieInfos?.value} />
 )
}