...
export default async function CalcFreight(params: params):Promise<calcFreightResponse> {
  ...
  try {
    ...
    if (res && 'message' in res && (res.message == "cep de destino inválido" || res.message == "frete não disponível" || res.message == "roupa não encontrada" || res.message == "peso maxímo atingido")) {  
      return {
        data: res.message,
        status: 400
      }
