...
import {
 ...
 cardRequest,
 clothing,
 responseErrorsRetryPayment,
 responseErrorsRetryPaymentType,
} from './payOrderInterfaces';
...
export default async function RetryPayment(
 ...
 try {
  ...
  if (res && 'message' in res && res.message) {
   ...
   if (
    responseErrorsRetryPayment.includes(
     res.message as responseErrorsRetryPaymentType,
    )
   ) {
    return res.message as responseErrorsRetryPaymentType;
   }
