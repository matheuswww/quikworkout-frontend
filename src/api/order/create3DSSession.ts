...
type create3DSSessionResponse =
 | statusCode
 | responseErrors
 | create3DSSessionData;

export default async function Create3DSSession(
 cookie: string,
 ...
 try {
  ...
  if (
   res &&
   'message' in res &&
   (res.message == 'cookie inválido' || res.message == 'contato não verificado')
  ) {
   return res.message;
  }
