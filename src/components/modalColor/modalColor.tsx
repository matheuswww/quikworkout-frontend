 ...
 return (
  <>
   <button
    ...
    type="button"
    ref={buttonToOpenModalRef}
    aria-label={`cor ${color} está selecionada, clique aqui para alterar a cor da roupa`}
   >
    <p>{color}</p>
...
export function ModalColor({
 ...
 return (
  <div
   ...
   style={{ backgroundColor: color ? color : '#19191d' }}
  >
   {mainColor && (
    <button
     type="button"
     ...
     aria-label={`${mainColor}, opção 1 de ${inventario?.length}`}
    >
     {mainColor}
    </button>
   )}
    ...
    return (
     !corPrincipal && (
      <button
       ...
       aria-label={`${cor}, opção ${index + 2} de ${inventario.length}`}
      >
       {cor}
      </button>
     )
