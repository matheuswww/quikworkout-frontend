...
} from '@/api/clothing/getOrderDetail';
...
export default function FinishPurchaseForm({ ...props }: props) {
 ...
 const [retryPaymentData, setRetryPaymentData] =
  useState<getOrderDetailResponse | null>(null);
 const [paymentTypeRetryPayment, setPaymentTypeRetryPayment] = useState<
  string | null
 >(null);
 ...
 useEffect(() => {
  if (!end) {
   ...
   (async function () {
    ...
    if (
     props.retryPaymentId == undefined ||
     (props.paymentTypeRetryPayment != 'CARTAO_CREDITO' &&
      props.paymentTypeRetryPayment != 'CARTAO_DEBITO' &&
      props.paymentTypeRetryPayment != 'BOLETO' &&
      props.paymentTypeRetryPayment != 'PIX')
    ) {
     if (
      (props.clothing_id == undefined ||
     ...
     const res = await GetClothingCart(
      cookie,
      cursor,
      props.clothing_id,
      props.color,
     ...
     setTotalPrice(
      Math.round(
       Math.abs(res.data.pedido.precoTotal - res.data.pedido.frete) * 100,
      ) / 100,
     );
  ...
  await PagSeguro.authenticate3DS(request)
   ...
   .then((result) => {
    if (result.status == 'CHANGE_PAYMENT_METHOD') {
     setResponseError3ds(
      'autenticação 3DS negada pelo pagbank,escolha outro meio de pagamento',
     );
    }
    if (result.status == 'AUTH_NOT_SUPPORTED') {
     setResponseError3ds(
      'autenticação 3DS indisponível para este cartão,escolha outro meio de pagamento',
     );
    }
 ...
 async function handleSubmit(event: SyntheticEvent) {
  ...
  if (
   paymentType == null ||
   (card == null && boleto == null && paymentType != 'pix')
  ) {
  ...
  if (!retryPaymentData && !retryPaymentId) {
   ...
   if (address != null && paymentType != null && data && data.clothing) {
    ...
    if (
     res == 'roupa não encontrada' ||
     res == 'a quantidade do pedido excede o estoque'
    ) {
     setResponseError(
      'parece que uma das suas roupas está indisponível, volte a sua bolsa e exclua ou altere esta roupa',
     );
     ...
     res == 'preço calculado não é igual ao esperado' ||
     res == 'não foi possivel salvar o pedido'
    ) {
     setPopupError(true);
     if (res == 'preço calculado não é igual ao esperado') {
      setResponseError(
       'parece que houve uma alteração nos preços,recarregue a página e tente novamente',
      );
      setEnd(false);
     }
    } else {
     ...
     if (
      typeof res == 'string' &&
      (responseErrorsPayOrder.includes(res as responseErrorsPayOrderType) ||
       quantityErrorPattern.test(res))
     ) {
      setResponseError(res);
     }
    }
   ...
   if (
    res == 'novo tipo de pagamento não pode ser pix' ||
    res == 'não foi possível salvar o pedido' ||
    res == 'não é possível pagar um pedido cancelado com pix' ||
    res == 500
   ) {
    setPopupError(true);
   } else if (
    res == 'ocorreu uma alteração nos preços das roupas' ||
    res == 'preço calculado não é igual ao esperado'
   ) {
    setResponseError(
     'não é possivel tentar pagar novamente, o preço foi alterado',
    );
   } else if (res == 'cep de destino inválido') {
    setResponseError(res);
   } else if (res == 'peso máximo atingido') {
    setResponseError(
     'não é possível tentar pagar novamente, o peso excede o máximo de entrega',
    );
   } else if (res == 'a quantidade do pedido excede o estoque') {
    setResponseError(
     'não é possível tentar pagar novamente, o estoque não é suficiente',
    );
   } else if (
    res == 'pedido está sendo processado' ||
    res == 'pedido já está pago' ||
    res == 'erro ao deletar carrinho'
   ) {
    router.push('/usuario/meus-pedidos');
    return;
   } else {
 ...
 return (
  <>
   ...
   <main
    className={`${styles.main} ${load && `${styles.opacity}`} ${(data?.status == 500 || data?.status == 404 || retryPaymentData?.status == 404 || retryPaymentData?.status == 500 || addressStatus == 500) && styles.mainHeight}`}
   >
    <div className={styles.container}>
     {(data?.status == 200 ||
      (retryPaymentData == null && data == null) ||
      retryPaymentData?.status == 200 ||
      (retryPaymentData == null && data == null)) &&
      addressStatus != 500 && (
       <h1 className={styles.title}>Finalizar compra</h1>
      )}
     {((data?.status == 200 && data.clothing) ||
      (retryPaymentData?.status == 200 && paymentTypeRetryPayment)) &&
     addressStatus != 500 ? (
      <>
       ...
       <Payment
        ...
        responseError3ds={responseError3ds}
        setIdTo3ds={setIdTo3ds}
        addressRef={addressRef}
        address={address}
        retryPayment={paymentTypeRetryPayment}
        paymentRef={paymentRef}
        responseError={responsePaymentError}
        setBoleto={setBoleto}
       ...
       />
       {(!paymentTypeRetryPayment || paymentTypeRetryPayment == 'BOLETO') && (
        <Address
         addressForm={addressForm}
         setAddressForm={setAddressForm}
         setAdressStatus={setAddressStatus}
         setLoad={setLoad}
         cookieName={props.cookieName}
         cookieVal={props.cookieName}
         addressRef={addressRef}
       ...
       <form onSubmit={handleSubmit}>
        <Products
         ...
         setTotalPriceWithFreight={setTotalPriceWithFreight}
         totalPriceWithFreight={totalPriceWithFreight}
         retryPaymentData={retryPaymentData}
         responseError={responseError}
         freight={freight}
         clothing={data?.clothing}
         totalPrice={formatPrice(Math.round(totalPrice * 100) / 100)}
        />
       </form>
      </>
     ) : (
     ...
     {(data?.status == 404 || retryPaymentData?.status == 404) &&
      addressStatus != 500 && (
       <div>
        <p className={styles.notFound} style={{ marginTop: '25px' }}>
         {'Pedido não encontrado'}
        </p>
        <Link
         style={{ marginLeft: '10px' }}
         href={`${paymentTypeRetryPayment && retryPaymentId ? '/usuario/minha-bolsa' : '/'}`}
         className={styles.seeClothing}
        >
         {paymentTypeRetryPayment && retryPaymentId
          ? 'Ver carrinho'
          : 'Ver roupas'}
        </Link>
       </div>
