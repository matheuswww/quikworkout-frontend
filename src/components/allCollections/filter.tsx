...
export default function Filter() {
 ...
 function handleForm(data: FormProps) {
  ...
  if (data.maxPrice)
   params.append(
    'precoMaximo',
    data.maxPrice.includes(',')
     ? data.maxPrice.replace(',', '.')
     : data.maxPrice,
   );
  if (data.minPrice)
   params.append(
    'precoMinimo',
    data.minPrice.includes(',')
     ? data.minPrice.replace(',', '.')
     : data.minPrice,
   );
 ...
 return (
  ...
  >
   ...
   <div className={styles.gender}>
    <label htmlFor="m" className={styles.m}>
     masculino
    </label>
    <input type="checkbox" id="m" {...register('m')} />
    <label htmlFor="f" className={styles.f}>
     feminino
    </label>
   ...
   {errors.maxPrice?.message && (
    <p className={styles.error}>{errors.maxPrice.message}</p>
   )}
   <label htmlFor="minPrice">Preço minímo</label>
   <input {...register('minPrice')} type="text" id="minPrice" />
   {errors.minPrice?.message && (
    <p className={styles.error}>{errors.minPrice.message}</p>
   )}
   ...
   >
    <span aria-hidden="true" id="closeSpan">
     x
    </span>
   </button>
  </form>
