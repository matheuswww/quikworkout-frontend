...
export default function FilterButton() {
  ...
  function close(event: MouseEvent) {
    ...
    if(form instanceof HTMLFormElement) {
      if(event.target instanceof HTMLElement) {
        if(form.contains(event.target) && event.target.id != "close" && event.target.id != "closeSpan") {
          return
        }
  ...
  return (
    <button className={styles.filterButton} onClick={handleClick}>Filtrar coleções</button>
  )
}