...
export default function CreateClothingForm({...props}:props) {
  ...
  useEffect(() => {
    if(inventory && inventory?.length > 0 && error == "é necessário pelo menos 1 inventário") {
      setError(null)
    }
  }, [inventory])
      ...
      <main className={`${styles.main} ${load && styles.opacity}`}>
        ...
        <section className={styles.section}>
          <form className={styles.form} onSubmit={handleSubmit(handleForm)}>
            ...
            <input {...register("descricao")} id="description" placeholder="descricação"/>
            {errors.descricao?.message && <p className={styles.error}>{errors.descricao?.message}</p>}
            <label htmlFor="price">Preço</label>
            <input {...register("preco")} type="text" id="price" placeholder="preço"/>
            ...
            <div className={styles.sex}>
              ...
              <input id="m" type="checkbox" checked={sex == "M"} onChange={() => setSex("M")} />
              <label htmlFor="f">F</label>
              <input id="f" type="checkbox" checked={sex == "F"} onChange={() => setSex("F")}/>
              <label htmlFor="unisex">UNISEX</label>
              <input id="unisex" type="checkbox" checked={sex == "UNISEX"} onChange={() => setSex("UNISEX")}/>
            </div>
            <button className={`${styles.addInventory} ${styles.button}`} disabled={load} type="button" onClick={() => handleModalClick(modalRef, buttonRef, buttonCloseRef, styles.active, "grid")} ref={buttonRef}>Adiconar novo inventário</button>
            {
                ...
                return (
                  <div key={i.cor+index}>
                    <div className={styles.show}>
                      ...
                      <input type="checkbox" className={styles.checkbox} checked={mainInventory == index} onChange={() => setMainInventory(index)} />
                        <p className={styles.p}>Cor: <span className={styles.val}>{i.cor}</span></p>
                        <p className={styles.p}>Descrição da imagem: <span className={styles.val}>{i.imgDesc}</span></p>
                        <p className={styles.p}>P: <span className={styles.val}>{i.p}</span></p>
                        <p className={styles.p}>M: <span className={styles.val}>{i.m}</span></p>
                        <p className={styles.p}>G: <span className={styles.val}>{i.g}</span></p>
                        <p className={styles.p}>GG: <span className={styles.val}>{i.gg}</span></p>
                          <div className={styles.show}>
                            <button className={styles.buttonExpand} type="button" onClick={() => handleArrowClick(index, "images", styles.displayNone)}>Ver imagens</button>
                            <Image src="/img/arrowUp.png" alt="expandir ver todas as imagens" width={30} height={30} className={`${styles.expand}`} onClick={() => handleArrowClick(index, "images", styles.displayNone)} id={`arrowUp_images_${index}`} />
                            <Image src="/img/arrowDown.png" alt="diminuir ver todas as imagens" width={30} height={30} className={`${styles.expand} ${styles.displayNone}`} onClick={() => handleArrowClick(index, "images", styles.displayNone)} id={`arrowDown_images_${index}`}/>
                          </div>
                          <div className={`${styles.images} ${styles.displayNone}`} id={`item_images_${index}`}>
                            {
                              i.images.map((image) => {
                                const imageUrl = URL.createObjectURL(image);
                                return <Image src={imageUrl} alt={i.imgDesc} width={80} height={85} key={imageUrl} className={styles.image}/>
                              })
                            }
                          </div>
                      </div>
            ...
            <button type="submit" className={styles.button} disabled={load}>Criar roupa</button>
          </form>
        </section>
      </main>
    </>
