...
export default function GetOrderAdmin({
 cookieName,
 cookieVal,
 updated,
}: props) {
 ...
 const [packageNumbers, setPackageNumbers] = useState<packageNumbers[] | null>(
  null,
 );
 ...
 return (
  <>
   ...
   <UpdateTrackingCodeForm
    ...
    cookieVal={cookieVal}
    closeRef={closeTrackingCodeRef}
    modalRef={trackingCodeRef}
   />
   <Filter
    load={load}
    closeRef={closeFilterRef}
    modalRef={filterRef}
    setLoad={setLoad}
   />
   <Menu cookieName={cookieName} cookieVal={cookieVal} />
   <main
    className={`${styles.main} ${load && styles.opacity} ${data?.status == 500 && styles.mainError}`}
   >
    <section>
     {data?.status == 500 && (
      <p>Ooops! Parece que houve um erro,tente recarregar a página</p>
     )}
     {data?.status != 500 && (
      <div className={styles.order}>
       ...
       <button
        ...
        onClick={() =>
         handleModalClick(
          filterRef,
          buttonToOpenModalRefFilter,
          closeFilterRef,
          styles.active,
          'flex',
         )
        }
        ref={buttonToOpenModalRefFilter}
       >
        Filtrar
       </button>
       {data?.order && (
        <p className={styles.p}>
         {end
          ...
          : `Ainda exitem pedidos a serem carregados`}
        </p>
       )}
       {data?.status == 404 && !data.order?.pedido && (
        <p className={`${styles.p}`}>Nenhum pedido foi encontrado</p>
       )}
       {load && !data && (
        <p className={styles.p}>carregando pedidos aguarde...</p>
       )}
      </div>
     )}
      ...
      return (
       <div className={styles.item} key={order.pedido_id}>
        ...
        <div className={`${styles.values}`}>
         ...
         <p className={order.status_pagamento == 'pago' ? styles.green : ''}>
         ...
         <p>
          {order.tipo_pagamento == 'DEBIT_CARD'
           ...
           : order.tipo_pagamento == 'CREDIT_CARD'
             ? 'cartão de crédito'
             : order.tipo_pagamento.toLowerCase()}
         </p>
        </div>
        <div className={`${styles.values}`}>
         <p>Rua: </p>
        ...
        >
         <Image
          ...
          width={20}
          height={21}
          alt="adicionar código de rastreio"
         />
        </button>
       </div>
      );
     ...
     {end && data?.status != 404 && data?.order?.pedido.length != 0 && (
      <p className={styles.end}>Todos os pedidos foram carregados</p>
     )}
    </section>
   </main>
  </>
