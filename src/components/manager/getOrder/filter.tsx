...
const schema = z.object({
  ...
  return fields.filter != "" && !isNaN(Number(fields.filter)) && Number(fields.filter) >= 0 && Number(fields.filter) <= 100000
}, {
  path: [ 'filter' ],
...
export default function Filter({closeRef,modalRef,setLoad,load}:props) {
  ...
  return (
    <form tabIndex={0} className={`${styles.modal}`} ref={modalRef} onSubmit={handleSubmit(handleForm)}>
      ...
      <input {...register("filter")} type="number" placeholder="filtrar" id="filter"/>
      {errors.filter?.message && <p className={styles.error}>{errors.filter.message}</p>}
      ...
      <button type="submit" disabled={load} className={`${styles.modalButton} ${styles.button}`}>Filtrar</button>
    </form>
  )
}