...
const schema = z.object({
  cancelationReason: z.string().min(3, "é necessário no minímo 3 caracteres").max(255, "é permitido no maxímo 255 caracteres")
})
...
export default function CancelShippingForm({setData,setLoad,setPopupError,data,cookieName,cookieVal}:props) {
  ...
  return (
    data?.order?.pedido[0].status_pagamento == "cancelado" && data.order.pedido[0].cancelamento == "" &&
    <form className={styles.form} onSubmit={handleSubmit(handleForm)}>
      <label htmlFor="reason">Motivo do cancelamento</label>
      <input {...register("cancelationReason")} placeholder="motivo do cancelamento" id="reason"/>
      {errors.cancelationReason?.message && <p className={styles.error}>{errors.cancelationReason.message}</p>}
