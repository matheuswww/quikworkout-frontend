...
export default function Clothing({data,index,cookieName,cookieVal,setLoad,setPopupError,success,load,setSuccess}: props) {
  ...
  const buttonToOpenModalDeleteClothingRef = useRef<HTMLButtonElement | null>(null)
  ...
  useEffect(() => {
    if (
      ...
      slidesWithControll(slide.current, images.current, undefined, styles.index, styles.active, styles.activeThumb)
    }
  },[color])
  ...
  function handleCheckBoxGender(val: string, index: number) {
    ...
    if(m instanceof HTMLInputElement && f instanceof HTMLInputElement && unisex instanceof HTMLInputElement) {
      if("M" != val && m.checked) {
        m.checked = false
      }
      if("F" != val && f.checked) {
  ...
  function handleLoad() {
    ...
    data.inventario.forEach(({corPrincipal}, indexInventory) => {
      if(corPrincipal) {
        const checkedInventory = document.querySelector("#mainColor"+index+indexInventory)
  ...
  async function handleForm(dataForm:FormProps, index:number) {
    ...
    if(formRef.current instanceof HTMLFormElement) {
      ...
      data.inventario.forEach(({...inventory},indexInventory) => {
        const mainColor = document.querySelector("#mainColor"+index+indexInventory)
      ...
      data.inventario.forEach(({...inventory}, indexInventory) => {
        ...
        if(mainColor instanceof HTMLInputElement && color instanceof HTMLInputElement && p instanceof HTMLInputElement && m instanceof HTMLInputElement && g instanceof HTMLInputElement && gg instanceof HTMLInputElement) {
          if(Number(p.value) == inventory.p && Number(m.value) == inventory.m && Number(g.value) == inventory.g && Number(gg.value) == inventory.gg && color.value == inventory.cor && (prevMainInventory == mainInventory)) {
            return
          }
          updateClothingInventory = true
          updateClothingInventoryParams.push({
            cor: inventory.cor,
            corPrincipal: ((prevMainInventory != mainInventory) && (mainInventory == indexInventory || prevMainInventory == indexInventory)) ? mainColor.checked : inventory.corPrincipal ,
      ...
      if(updateClothingInventory && !updateClothing) {
        handleUpdateClothingInventory(cookie, {
          id: data.id,
          atualizarInventario: updateClothingInventoryParams.map((i) => {
            return JSON.stringify(i)
          }),
          novoNomeCor: newColors,
        }, updateClothingInventoryParams, changedInventory)
      }
      if(updateClothing && updateClothingInventory) {
        handleUpdateClothing(cookie, updateClothingParams)
        handleUpdateClothingInventory(cookie, {
  ...
  async function handleUpdateClothing(cookie: string,updateClothing: updateClothingParams) {
    setLoad(true)
  ...
  async function handleUpdateClothingInventory(cookie: string, updateClothingInventoryParam: updateClothingInventoryParams, updateClothingInventory: Array<updateInventory>, changedInventory: Array<number>) {
    setLoad(true)
    const res = await UpdateClothingInventory(cookie, updateClothingInventoryParam)
    if(res == 401) {
      router.push("/manager-quikworkout/auth")
      return
    }
  ...
  return (
    <>
      <ModalColor inventario={data.inventario.map(({...infos}) => {
        return {
          cor: infos.cor,
      ...
      })} mainColor={mainColor} modalRef={modalColorRef} setColor={setColor} />
      <div className={styles.deleteClothing} ref={modalDeleteClothingRef} tabIndex={0}>
        <p>Tem certeza que deseja deletar esta roupa?</p>
        <button className={`${styles.button} ${styles.deleteButton}`} onClick={handleDeleteClothing}>deletar</button>
        <button aria-label="fechar" type="button" className={`${styles.close} ${styles.button}`} ref={closeModalDeleteClothingRef}><span aria-hidden="true">x</span></button>
      </div>
      <form className={styles.form} onLoad={handleLoad} onSubmit={handleSubmit((data:FormProps) => handleForm(data, index))} ref={formRef}>
        <div className={styles.images} ref={images} style={{display: data ? "initial" : "none"}}>
                ...
                imagens?.map(( src, index ) => {
                  return (
                    <li className={styles.product} key={src}>
                      { <SkeletonImage src={src} alt={alt} loading="lazy" width={290} height={460} className={styles.clothing} key={data.id} id={index.toString()} draggable={false} tabIndex={0}/> }
                    </li>
                  )
                })
              )
        ...
        <div className={styles.show}>
          <button className={styles.buttonExpand} type="button" onClick={() => handleArrowClick(index, "clothing", styles.displayNone)}>Ver dados da roupa</button>
          <Image src="/img/arrowUp.png" alt={`expandir ver dados da roupa ${index}`} width={30} height={30} className={`${styles.expand}`} onClick={() => handleArrowClick(index, "clothing", styles.displayNone)} id={`arrowUp_clothing_${index}`} />
          <Image src="/img/arrowDown.png" alt={`diminuir ver dados da roupa ${index}`} width={30} height={30} className={`${styles.expand} ${styles.displayNone}`} onClick={() => handleArrowClick(index, "clothing", styles.displayNone)} id={`arrowDown_clothing_${index}`}/>
        </div>
        <div className={`${styles.clothing} ${styles.displayNone}`} id={`item_clothing_`+index}>
          <p className={styles.p}>Cores:</p>
          <ChangeColor buttonToOpenModalRef={buttonToOpenModalColorRef} color={color} modalRef={modalColorRef} />
          <label htmlFor={"name"+index}>Nome:</label>
          <input type="text" defaultValue={data.nome} id={"name"+index} {...register("nome")}/>
          {errors.nome?.message && <p className={styles.error}>{errors.nome.message}</p>}
          <label htmlFor={"description"+index}>Descrição:</label>
          <input type="text" defaultValue={data.descricao} id={"description"+index} {...register("descricao")}/>
          {errors.descricao?.message && <p className={styles.error}>{errors.descricao.message}</p>}
          <label htmlFor={"category"+index}>Categoria:</label>
          <input type="text" defaultValue={data.categoria} id={"category"+index} {...register("categoria")}/>
          {errors.categoria?.message && <p className={styles.error}>{errors.categoria.message}</p>}
          <label htmlFor={"material"+index}>Material:</label>
          <input type="text" defaultValue={data.material} id={"material"+index} {...register("material")}/>
          {errors.material?.message && <p className={styles.error}>{errors.material.message}</p>}
          <label htmlFor={"price"+index}>Preço:</label>
          <input type="text" defaultValue={data.preco} id={"price"+index} {...register("preco")}/>
          {errors.preco?.message && <p className={styles.error}>{errors.preco.message}</p>}
          <div className={styles.checkbox}>
            <label>Sexo:</label>
            <label htmlFor={"M"+index}>M</label>
            <input type="checkbox" id={"M"+index} value={"M"} onChange={() => handleCheckBoxGender("M", index)}/>
            <label htmlFor={"F"+index}>F</label>
            <input type="checkbox" id={"F"+index} value={"F"} onChange={() => handleCheckBoxGender("F", index)}/>
            <label htmlFor={"UNISEX"+index}>UNISEX</label>
            <input type="checkbox" id={"UNISEX"+index} value={"UNISEX"} onChange={() => handleCheckBoxGender("UNISEX", index)}/>
          </div>
          <div className={styles.checkbox}>
          ...
          <div className={styles.show}>
            <button className={styles.buttonExpand} type="button" onClick={() => handleArrowClick(index, "inventory", styles.displayNone)}>Ver inventário(s) da roupa</button>
            <Image src="/img/arrowUp.png" alt={`expandir ver inventários da roupa ${index}`} width={30} height={30} className={`${styles.expand}`} onClick={() => handleArrowClick(index, "inventory", styles.displayNone)} id={`arrowUp_inventory_${index}`} />
            <Image src="/img/arrowDown.png" alt={`diminuir ver inventários da roupa ${index}`} width={30} height={30} className={`${styles.expand} ${styles.displayNone}`} onClick={() => handleArrowClick(index, "inventory", styles.displayNone)} id={`arrowDown_inventory_${index}`}/>
          </div>
          <div className={`${styles.displayNone} ${styles.item}`} id={`item_inventory_`+index}>
            {data.inventario.map((inventory, indexInventory) => {

              return (
                <Inventory index={index} indexInventory={indexInventory} inventory={inventory} inventoryLength={data.inventario.length} key={index+indexInventory}/>
              )
            })}
          </div>
