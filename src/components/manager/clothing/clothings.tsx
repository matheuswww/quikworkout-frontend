    ...
    <>
      ...
      <main className={`${styles.main} ${load && styles.opacity} ${(data?.status == 500 || data?.status == 404) && styles.mainError}`}>
        <section>
          ...
          {
            data?.clothing ? data.clothing.map((clothing,index) => {
              return <Clothing setSuccess={setSuccess} load={load} success={success} setLoad={setLoad} setPopupError={setPopupError} cookieName={props.cookieName} cookieVal={props.cookieVal} data={clothing} key={clothing.id} index={index} />
            })
          ...
          {data?.status == 404 && 
            <div>
              ...
              <Link className={styles.link} href={"/manager-quikworkout/criar-roupa"}>Criar roupa</Link>
            </div>
          }
