...
export default function MyOrder({ cookieName, cookieVal }: props) {
    ...
    if (res.data instanceof Array && res.status == 200) {
     ...
     for (let i = 0; i <= res.data.length - 1; i++) {
      if (
       ...
       const res2 = await getOrderDetail(
        res.data[i].tipo_pagamento,
        res.data[i].pedido_id,
       );
       if (res2) {
        if (
         res2.data &&
         typeof res2.data == 'object' &&
         res2.data.informacoesPagamento &&
         'qrcode' in res2.data.informacoesPagamento
        ) {
         array.push({
          card: null,
          boleto: null,
 ...
 async function handlePaymentInfoClick(
  index: number,
  type: string,
  pedido_id: string,
  tipoPagamento: string,
  ...
  if (
   paymentInfo[index].card ||
   paymentInfo[index].boleto ||
   paymentInfo[index].pix
  ) {
  ...
  if (res) {
   setPaymentInfo((pi) => {
    if (typeof res.data == 'object' && res.data != null && pi[index]) {
     pi[index] = {
      card:
       'bandeira' in res.data.informacoesPagamento
        ? res.data.informacoesPagamento
        : null,
      boleto:
       'dataVencimento' in res.data.informacoesPagamento
        ? res.data.informacoesPagamento
        : null,
      pix:
       'qrcode' in res.data.informacoesPagamento
        ? res.data.informacoesPagamento
        : null,
     };
    }
 ...
 return (
  <>
   ...
   <main className={`${styles.main} ${load && styles.opacity}`}>
    <section
     className={`${styles.section} ${(status == 404 || status == 500) && styles.sectionHeight}`}
    >
        ...
        <div key={infos.pedido_id} className={`${styles.item}`}>
         ...
         <div className={styles.values}>
          ...
          <p>
           R$
           {formatPrice(
            Math.round((infos.precoTotal - infos.frete) * 100) / 100,
           )}
          </p>
         </div>
         <div className={styles.values}>
         ...
         {infos.status_pagamento == 'pagamento não solicitado' &&
          paymentInfo[i]?.pix &&
          paymentInfo[i].pix?.qrcode &&
          paymentInfo[i].pix?.dataExpiracao == 'não expirada' && (
           <div className={`${styles.values} ${styles.infos} ${styles.qrcode}`}>
            <p>QR CODE para pagamento: </p>
            <QRCODE
             src={paymentInfo[i].pix?.qrcode as string}
             alt="qr code para pagamento via pix"
             width={220}
             height={220}
            />
            <button
             onClick={(event: SyntheticEvent) => copyLink(event, i)}
         ...
         infos.status_pagamento != 'cancelado' ? (
          <>
           {infos.pacotes.length > 1 && (
              ...
              <div className={styles.packageInfo}>
               {infos.pacotes.length > 1 ? (
                <button
                 ...
                 onClick={() =>
                  handleArrowClick(id, 'package', styles.displayNone)
                 }
                >
                 Rastreio do pacote {v + 1}
                </button>
               ) : (
                <button
                 className={styles.buttonExpand}
               ...
               <Expand
                src="/img/arrowUp.png"
                alt="expandir informações de rastreio"
                width={30}
                height={30}
                ...
                id={`arrowUp_package_${id}`}
               />
               <Expand
                src="/img/arrowDown.png"
                alt="diminuir informações de rastreio"
                width={30}
                height={30}
         ...
         <div className={styles.addressInfo}>
          <button
           className={styles.buttonExpand}
           onClick={() => handleArrowClick(i, 'address', styles.displayNone)}
          >
           Informações de endereço e contato
          </button>
          <Expand
           src="/img/arrowUp.png"
           alt="expandir informações de endereço e contato"
           width={30}
           height={30}
           className={`${styles.expand}`}
           onClick={() => handleArrowClick(i, 'address', styles.displayNone)}
           id={`arrowUp_address_${i}`}
          />
          <Expand
           src="/img/arrowDown.png"
           alt="diminuir informações de endereço e contato"
           width={30}
           height={30}
           className={`${styles.expand} ${styles.displayNone}`}
           onClick={() => handleArrowClick(i, 'address', styles.displayNone)}
           id={`arrowDown_address_${i}`}
          />
         </div>
         {
          <div
           id={`item_address_${i}`}
           className={`${styles.displayNone} ${styles.address}`}
          >
           <div className={`${styles.values} ${styles.infos}`}>
            <p>Email: </p>
            <p>{infos.email}</p>
         ...
         infos.status_pagamento != 'pagamento não solicitado' ? (
          <div
           className={`${styles.payment} ${styles.displayNone}`}
           id={`item_paymentInfo_${i}`}
          >
           <div className={`${styles.values} ${styles.infos}`}>
          ...
          >
           ...
           <div className={`${styles.values} ${styles.infos}`}>
            ...
            <Link
             className={styles.link}
             href={paymentInfo[i].boleto?.pdf as string}
             target="_blank"
             download="boleto em pdf"
            >
             baixar boleto
            </Link>
           </div>
           ...
           {paymentInfo[i].boleto?.mensagem != 'SUCESSO' && (
            <div className={`${styles.values} ${styles.infos}`}>
             <p>Mensagem: </p>
          ...
          paymentInfo[i]?.pix && (
           ...
           >
            ...
            {paymentInfo[i].pix?.tax_id && (
             <div className={`${styles.values} ${styles.infos}`}>
              <p>Cpf/Cnpj: </p>
              <p>{paymentInfo[i].pix?.tax_id}</p>
             </div>
            )}
            {paymentInfo[i].pix?.mensagem != 'SUCESSO' &&
             paymentInfo[i].pix?.mensagem && (
              <div className={`${styles.values} ${styles.infos}`}>
               <p>Mensagem: </p>
           ...
           <React.Fragment key={'pac' + j}>
            <div className={styles.clothingInfo}>
             <button
              ...
              key={`item_button_${id}`}
             >
              Roupa(s) {infos.pacotes.length > 1 && `do pacote ${j + 1}`}
             </button>
             <Expand
              src="/img/arrowUp.png"
              alt="expandir informações das roupas do pedido"
              width={30}
              height={30}
              className={styles.expand}
              onClick={() =>
             ...
             <Expand
              src="/img/arrowDown.png"
              alt="diminuir informações das roupas do pedido"
              width={30}
              height={30}
              className={`${styles.expand} ${styles.displayNone}`}
              onClick={() =>
         ...
         {infos.tipo_pagamento == 'PIX' &&
          paymentInfo[i]?.pix?.dataExpiracao == 'expirada' && (
           <div>
            <p className={styles.error}>Pix expirado</p>
            <Link
             href={`/finalizar-compra?retry_payment_id=${infos.pedido_id.substring(5)}&paymentType=${infos.tipo_pagamento}`}
             className={styles.link}
            >
             Gerar novo pix
            </Link>
         ...
         {infos.status_pagamento == 'recusado' &&
          !paymentInfo[i]?.card?.mensagem.includes('não tente novamente') && (
           <div>
            <p className={styles.error}>Pagamento recusado</p>
            <Link
             href={`/finalizar-compra?retry_payment_id=${infos.pedido_id.substring(5)}&paymentType=${infos.tipo_pagamento == 'CREDIT_CARD' ? 'CARTAO_CREDITO' : 'CARTAO_DEBITO'}`}
             className={styles.link}
            >
             Pagar novamente
            </Link>
     ...
     <span
      aria-hidden={true}
      id="final"
      className={`${getOrder && styles.show}`}
     ></span>
     {newPageLoad && (
