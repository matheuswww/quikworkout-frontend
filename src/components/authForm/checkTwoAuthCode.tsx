    ...
    <>
      ...
      <main className={`${styles.main} ${load && styles.lowOpacity}`}>
        <form className={styles.form} onSubmit={handleSubmit(handleForm)}>
          ...
          <input {...register("code")} id="code" type="number" placeholder="insira seu código" style={{width: "100%"}} />
          ...
          <button type="submit" className={`${load && styles.loading}`} style={{width: "100%"}}>{load ? "Carregando..." : "Enviar código"}</button>
        </form>
      </main>
    </>
  )
