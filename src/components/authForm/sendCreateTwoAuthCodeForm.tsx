...
export default function SendCreateTwoAuthCodeForm({...props}: props) {
  ...
  async function handleForm(data: FormProps) {
    ...
    if(res == 500) {
      setPopUpError(true)
    } else if (res == "contato já utilizado para autenticação" || res == "este email já é utilizado para sua autenticação" || res == "este telefone já é utilizado para sua autenticação" || res == "senha errada") {
      setError(res)
